...
public class SpriteBatch {
	...
	// --Constants--//
	final static int VERTEX_SIZE = 5; // Vertex Size (in Components) ie.
										// (X,Y,U,V,M), M is MVP matrix index
	...
	private float[] uMVPMatrices = new float[GLText.CHAR_BATCH_SIZE * 16]; // MVP
																			// matrix
																			// array
																			// to
																			// pass
	...
	private float[] mMVPMatrix = new float[16]; // used to calculate MVP matrix
												// of each sprite
	...
	public SpriteBatch(int maxSprites, Program program) {
		...
		short[] indices = new short[maxSprites * INDICES_PER_SPRITE]; // Create
																		// Temp
																		// Index
																		// Buffer
		int len = indices.length; // Get Index Buffer Length
		short j = 0; // Counter
		for (int i = 0; i < len; i += INDICES_PER_SPRITE, j += VERTICES_PER_SPRITE) { // FOR
																						// Each
																						// Index
																						// Set
																						// (Per
		...
		mMVPMatricesHandle = GLES20.glGetUniformLocation(program.getHandle(),
				"u_MVPMatrix");
	}
	...
	public void endBatch() {
		if (numSprites > 0) { // IF Any Sprites to Render
			...
			GLES20.glUniformMatrix4fv(mMVPMatricesHandle, numSprites, false,
					uMVPMatrices, 0);
			GLES20.glEnableVertexAttribArray(mMVPMatricesHandle);

			vertices.setVertices(vertexBuffer, 0, bufferIndex); // Set Vertices
																// from Buffer
			vertices.bind(); // Bind Vertices
			vertices.draw(GLES20.GL_TRIANGLES, 0, numSprites
