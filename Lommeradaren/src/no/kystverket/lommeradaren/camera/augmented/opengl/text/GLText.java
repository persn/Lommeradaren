...
public class GLText {
	...
	public final static int CHAR_BATCH_SIZE = 24; // Number of Characters to
													// Render Per Batch
	...
	// --Constructor--//
	// D: save program + asset manager, create arrays, and initialize the
	// members
	public GLText(Program program, AssetManager assets) {
		...
		batch = new SpriteBatch(CHAR_BATCH_SIZE, program); // Create Sprite
															// Batch (with
															// Defined Size)
	...
	public void begin(float alpha, float[] vpMatrix) {
		begin(1.0f, 1.0f, 1.0f, alpha, vpMatrix); // Begin with White (Explicit
													// Alpha)
	}
	...
	// R: [none]
	public void draw(String text, float x, float y, float z, float angleDegX,
		...
		for (int i = 0; i < len; i++) { // FOR Each Character in String
			...
			// TODO: optimize - applying the same model matrix to all the
			// characters in the string
			batch.drawSprite(letterX, letterY, chrWidth, chrHeight, charRgn[c],
					modelMatrix); // Draw the Character
			letterX += (charWidths[c] + spaceX) * scaleX; // Advance X Position
															// by Scaled
															// Character Width
		}
	}
